...
0. **`pathSum_fst_le` DONE** (axiom-clean). NEXT smallest probe: the **good-column depth-sourcing
   lemma** — from `(∀i, v i∈hold.support)` + `adv<0.9m` + `Cthr≥10g` + `m≤n/2` derive
   `∀p≤P, (q₀+pathSum v p).1+g ≤ n/2` (uses `pathSum_fst_le`, `hold_support_fst_pos` only for the
   `adv` def if needed, and ℕ arithmetic). Standalone, clean, feeds step 1.
1. **Pointwise split** (on good column): `ofReal(1_{myNw≤K}) ≤ ofReal(1_{reach R ∧ cumWhite≤K+1})
   + Σ_{p∈range(P+1)} indicator(bigTriangleSet F ⌊4^{κA}(1+p)³⌋)(phase pt) + ofReal(1_{adv≥0.9m})`,
   via `few_white_pointwise_dichotomy` @A'=κA (depth from step 0 on good col; on bad col the
   `1_{adv≥0.9m}` term ≥ the LHS directly). `bigTriangle_of_encounter` turns E∗ triangle → bigTri
   membership (indicator ≥1 at witnessing p).
