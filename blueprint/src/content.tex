...
Prop~1.17's remaining trust runs only through \texttt{white\_cos\_bound} (node~\ref{X2})
and the Prop~7.8 chain.
\end{definition}
...
\begin{lemma}[Many triangles \(\Rightarrow\) many white points; Lemma 7.9]\label{X9}
  \lean{many_triangles_white, EncState, encStep, encVal, encExpect}
  \uses{X4, X8}
  \leanok
...
(b) the \(2\varepsilon\) constant commits the proof to certifying \(p_0 \ge 1/2\) --- node
\ref{X8}'s kernel is pinned only \(\exists p_0 > 0\) (Monte Carlo \(\approx 0.99\), so
expected fine, but it is a certification burden --- else re-loosen to an \(\exists C\) form).
...
\texttt{encExpect\_block\_le}, coupling \texttt{encExpect\_anti} (all axiom-checked pass 9);
open: the two-level \(Y/Z\) induction + a (7.59)-shaped white-exit variant (node~\ref{X8}).
\lapsrisk{5--10}{medium --- statement pinned, bridge proved, paper gap routed around}{70\% (2026-07-12 judge pass 9: gap confirmed real; corrected route sound; p$_0\ge$1/2 certification burden added)}
\end{lemma}
