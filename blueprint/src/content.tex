...
\emph{Statement pinned + ratified 2026-07-12 (judge pass 8, lap 52)}: the stopping-time data
\((t_i, \Delta_i, r)\) is a left fold (\texttt{EncState}/\texttt{encStep}: position, clearing
barrier, encounter count, running + banked white counts), and \texttt{many\_triangles\_white}
states \(\texttt{encExpect} \le e^{\varepsilon}\) for all horizons \(T\), all \(R \ge 1\), all
\(\varepsilon \le \varepsilon_0\).  Three ratified deltas vs the paper: \emph{finite horizon}
\(\forall T\) (D6 --- no infinite product measure; route-trigger T1 did NOT fire);
\emph{\(\exists\varepsilon_0\,\forall\varepsilon\le\varepsilon_0\)} in place of the fixed
section constant (rider: verify at node~\ref{X11} that the p.55 consumption chooses \(R\)
after \(\varepsilon\)); \emph{index shift} (triangle/black read at the phase point
\((q_1-1, q_2)\), matching the ratified Case-2 forms).  The geometric prerequisites
(\texttt{not\_mem\_two}, \(\Delta(q)\)) and the head-peel recursion \texttt{encExpect\_succ}
(the p.51 conditioning skeleton) are already \emph{proved}; open: the \(R\)-induction + the
path\(\to\)\texttt{fpDist} bridge, closed by \texttt{fpDist\_white\_exit} (node~\ref{X8}).
\lapsrisk{5--10}{medium --- statement pinned, head-peel recursion proved}{75\% (2026-07-12 judge: encoding ratified vs pp.50--51; R-induction + bridge open)}
\end{lemma}
