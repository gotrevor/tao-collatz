...
kernel is the node's sole remaining input.  \emph{Kernel mass demand (pass 16)}: the
proof exhibits \(\varepsilon_0 = \min(1/100, (2p_1-1)/2)\), so the pin's bare
\(1/2 < p_0\) leaks the \(\varepsilon_0 \ge 10^{-4}\) consumer floor --- the kernel
re-pin must certify \(51/100 \le p_0\) (numerics \(\approx 0.99\); pre-authorized as a
ratification-preserving strengthening).
\lapsrisk{2--4}{medium --- kernel-only: the deep white-exit input (with the 51/100 mass numeral)}{80\% (2026-07-12 judge pass 16: Y/Z induction verified; trail machine-checked)}
\end{lemma}
