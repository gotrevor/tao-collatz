...
clean); the sole remaining tail is \texttt{fpDist\_any\_triangle\_le} (family-triangle
mass \(\le 1/8\) --- the quantitative (7.50)/(7.51) exclusion step, ratified as a route
decomposition).
\lapsrisk{1--3}{low --- one tail: the 1/8 family-triangle mass bound (X6+S3 machinery, same as its proved siblings)}{85\% (2026-07-12 judge pass 17)}
\end{lemma}

\begin{lemma}[Large triangles rarely encountered; Lemma 7.10]\label{X10}
  \lean{triangle_encounter_le, fpDistPlus, bigTriangleSet, TriangleFamily.not_mem_two}
  \uses{X3, X6, S3}
  \leanok
...
the same trust status as \texttt{fpDist} itself.  Quantifying \(\forall A>0\) with uniform
\(C,c\) strengthens the paper's fixed-\(A\) form, which the p.54 union bound requires.  The
(7.65) apex-interval disjointness step (\texttt{TriangleFamily.not\_mem\_two}) is already
\emph{proved} from node~\ref{X3}'s separation.
\lapsrisk{10--20}{high --- risk kernel 3b, statement pinned}{70\% (2026-07-12 judge: statement ratified vs pp.51--54; separated-\(\Sigma\) summation still open)}
\end{lemma}
...
(7.67) encounter, and X10's \(\forall A\)-uniform consumption at the p.54 union bound.
