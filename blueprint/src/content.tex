...
\begin{lemma}[Many triangles \(\Rightarrow\) many white points; Lemma 7.9]\label{X9}
  ...
  \uses{X4, X8}
  \notready
\textbf{RATIFICATION SUSPENDED (judge pass 13, 2026-07-12).}  The lap-55 reflection
challenges the \emph{truth} of the pinned \(e^{2\varepsilon}\) form and the judge concurs
(\(\sim\)85\%): \(W\) is strip-restricted, Lemma 7.4's confinement keeps triangles only
\(\sim 0.92\) deep (not \(C_{\mathrm{thr}}\)-deep), so an adversarial family stacked in the
edge strip chains up to \(C_{\mathrm{thr}}\) encounters whose \(e^{\varepsilon}\) payments
have no white-exit compensation --- a pathwise \(e^{\varepsilon C_{\mathrm{thr}}} \gg
e^{2\varepsilon}\) factor.  (The paper's own all-starts Lemma 7.9 inherits the same
overreach through ``(7.59) by repeating (7.51)'' --- literature hole \#6, sibling of \#5;
the Case-3 consumer only produces deep encounters, so the theorem is unaffected.)
Fix pending (depth-gated fold preferred, \(\exists C\) re-pin fallback); statement-leanok
returns on re-ratification of the re-pin.
...
\lapsrisk{4--8}{high --- statement TRUTH challenged (near-edge regime); re-pin pending}{60\% (2026-07-12 judge pass 13: challenge concurred; both proposed fixes pre-assessed consumer-sound; ledger core survives either)}
\end{lemma}
