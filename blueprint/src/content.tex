...
%
%  STATUS DISCIPLINE (this pass):
%    * Every node carries `\notready`.  Nothing is proved yet (Phase-A skeleton only), and
%      this authoring pass verifies no build; a machine reconciler will flip statuses to
%      `\leanok` later from the actual Lean state / `#print axioms`.
...
%    * Every incomplete (`\notready`) node carries a `\lapsrisk{laps}{risk}{conf}` campaign
...
%      Drop the line when a node flips to `\leanok`; re-sync numbers from the ledger.
% ======================================================================================
