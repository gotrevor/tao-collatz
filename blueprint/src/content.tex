...
%    * Statement-`\leanok` is set on the 16 nodes whose `\lean{}`-bound statements are
%      landed, compiled, and judge-ratified (S1 S2 S4 C1--C7 C9 C10 X2 X3 X4 X11).  The
%      9 nodes with no canonical Lean name yet (S3 C8 X1 X5--X10) keep `\notready` until
%      their statements are pinned.  PROOF-`\leanok` (green background) stays off until
%      the g-i machine reconciler (BlueprintAudit / `#print axioms`) is transplanted --
%      no proof status is hand-maintained.
...
% TODO-PRECISE: exact Gaussian window / constant in Lemma 2.2(i)(ii) not transcribed here.
\lapsrisk{20--40}{high --- risk kernel 1}{70\%}
\end{lemma}
...
\lapsrisk{15--30}{high --- risk kernel 2}{75\% (2026-07-10: exact-arithmetic $l_*/j_*$ decomposition validated at three instances incl.\ giant triangles; set-separation statement ratified)}
\end{lemma}
...
\begin{lemma}[First-passage location of the holding walk; Lemma 7.7]\label{X6}
  \uses{S3, X5}
  \notready
\textbf{Lemma 7.7}: the distribution of the location at which the \(\mathrm{Hold}\) renewal
walk first crosses a prescribed level is close to uniform on the relevant scale, quantitatively.
...
(node~\ref{X2}) already delivers the required decay.
% TODO-PRECISE: no canonical Lean name for Prop 7.8 in SKELETON-SPEC yet.
\lapsrisk{4--8}{low}{85\%}
\end{proposition}
