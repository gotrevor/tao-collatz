...
- **Box docstring nits** (box's to fix): `White.lean:11` + `Reduction.lean:12`
  ...
  Lemma 2.1's membership half (`1 ≤ valVec N n i`, the paper's
  `~a⁽ⁿ⁾ ∈ (ℕ+1)ⁿ`) has no companion lemma — add the one-liner via
  `syr_iterate_odd` if any consumer needs valVec itself to satisfy an
  entries-≥1 hypothesis (pass 21).
