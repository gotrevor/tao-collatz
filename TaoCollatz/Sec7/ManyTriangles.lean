...
theorem encExpect_nonneg {n ξ : ℕ} (F : TriangleFamily n ξ) (R g : ℕ) (ε : ℝ)
  ...
  tsum_nonneg fun _ => mul_nonneg ENNReal.toReal_nonneg (encVal_pos ε R _).le
  ...
  | _ s IH =>
    ...
    have hboundR : ∀ d : ℕ × ℤ, (hold d).toReal
      ...
      fun d => mul_le_mul_of_nonneg_left
        (tsum_toReal_mul_le _ f hg0 hgB) ENNReal.toReal_nonneg
...
theorem encChainX_le_exp {ε p₀ : ℝ} (hε : 0 ≤ ε)
    (hsmall : (1 - p₀) * (Real.exp ε + 1) ≤ 1) :
...
theorem encounter_two_mass_bound {ε p₀ d : ℝ} (hε : 0 ≤ ε)
  ...
  have hX1 := one_le_encChainX hε hp1 hsmall
...
theorem encExpect_entered_le {n ξ : ℕ} (F : TriangleFamily n ξ) (g : ℕ) (ε p₀ : ℝ)
  ...
  classical
  have hX1 : 1 ≤ encChainX ε p₀ := one_le_encChainX hε hp1 hsmall
    ...
    have hstep : ∀ e : ℕ × ℤ, (s : ℤ) < e.2 → ∀ T' : ℕ, T' < T →
      ...
      · -- no instant re-encounter: the exit wanders with its whiteness credit
        ...
        · have hsx : encStep F (ρ + 1) g ⟨w, t.2.1, 0, 0, 0⟩ e
            ...
            simp [hW]
          rw [hsx]
          have hwander := encExpect_wander_le F ρ g ε hε (encChainX ε p₀)
...
/-- The threshold of `gaussian_col_tail`, symbolic (big-C campaign, step 2):
`max 25 (⌈5·ln(8D)/γ + 3⌉ + 1)` with `γ = min(c, c²/20)` and
`D = C'·((1−e^{−c²/20})⁻¹ + (1−e^{−c})⁻¹) + 1`. -/
noncomputable def T_gaussColTail (c C' : ℝ) : ℕ :=
  max 25 (Nat.ceil (5 * Real.log (8
      * (C' * ((1 - Real.exp (-(c ^ 2 / 20)))⁻¹ + (1 - Real.exp (-c))⁻¹) + 1))
    / min c (c ^ 2 / 20) + 3) + 1)
...
`_at` sibling at `T_gaussColTail c C'` (big-C campaign, step 2). -/
theorem gaussian_col_tail_at {c C' : ℝ} (hc : 0 < c) (hC' : 0 ≤ C') :
    ∀ s m : ℕ, T_gaussColTail c C' ≤ m →
...
/-- `gaussian_col_tail`, original `∃`-form: delegates to the `_at` sibling at
`T_gaussColTail c C'`. -/
theorem gaussian_col_tail {c C' : ℝ} (hc : 0 < c) (hC' : 0 ≤ C') :
    ∃ Cthr : ℕ, ∀ s m : ℕ, Cthr ≤ m →
      ...
      ∑' j : ℕ, (if m < j then
          C' * (Gweight (1 + (s : ℝ)) (c * ((j : ℝ) - (s : ℝ) / 4))
                  / Real.sqrt (1 + (s : ℝ))) else 0) ≤ 1 / 8 :=
  ⟨T_gaussColTail c C', gaussian_col_tail_at hc hC'⟩
...
theorem fpDist_out_of_strip_le_core (c C' : ℝ) (Cthr : ℕ)
    (_hc : 0 < c) (_hC'pos : 0 < C')
    (hcol : ∀ (s j : ℕ),
...
/-- The threshold of `fpDist_out_of_strip_le`, symbolic (big-C campaign, step 2):
`T_gaussColTail` at the explicit column-marginal constants. -/
noncomputable def T_outStrip : ℕ := T_gaussColTail c_fpLocation C_fpCol

/-- `fpDist_out_of_strip_le`, `_at` sibling: `fpDist_out_of_strip_le_core` at
(`c_fpLocation`, `C_fpCol`, `T_outStrip`). -/
theorem fpDist_out_of_strip_le_at :
    ∀ n ξ : ℕ, ¬ 3 ∣ ξ →
      ∀ F : TriangleFamily n ξ, ∀ m : ℕ, T_outStrip ≤ m → m ≤ n / 2 →
      ∀ l : ℤ, 1 ≤ n / 2 - m →
      ∀ t ∈ F.T, (n / 2 - m - 1, l) ∈ triangle t.1 t.2.1 t.2.2 →
      ∀ s : ℕ, (s : ℤ) = t.2.1 - l →
      ∑' e : ℕ × ℤ, (fpDist s e).toReal
        * Set.indicator (outStripSet n) 1 (n / 2 - m + e.1, l + e.2) ≤ 1 / 8 := by
  have h := fpDist_out_of_strip_le_core c_fpLocation C_fpCol
    (T_gaussColTail c_fpLocation C_fpCol) c_fpLocation_pos C_fpCol_pos
    fpDist_col_le_explicitC (gaussian_col_tail_at c_fpLocation_pos C_fpCol_pos.le)
  unfold T_outStrip
  exact h

/-- **Out-of-strip tail** (⅛ of the (7.50) budget), original `∃`-form: delegates
to the `_at` sibling at `T_outStrip`. -/
theorem fpDist_out_of_strip_le :
    ∃ Cthr : ℕ, ∀ n ξ : ℕ, ¬ 3 ∣ ξ →
      ...
      ∀ s : ℕ, (s : ℤ) = t.2.1 - l →
      ∑' e : ℕ × ℤ, (fpDist s e).toReal
        * Set.indicator (outStripSet n) 1 (n / 2 - m + e.1, l + e.2) ≤ 1 / 8 :=
  ⟨T_outStrip, fpDist_out_of_strip_le_at⟩
...
theorem fpDist_any_triangle_le_at :
    ∀ n ξ : ℕ, ¬ 3 ∣ ξ →
      ∀ F : TriangleFamily n ξ, ∀ m : ℕ, 0 ≤ m → m ≤ n / 2 →
      ∀ l : ℤ, 1 ≤ n / 2 - m →
      ∀ t ∈ F.T, (n / 2 - m - 1, l) ∈ triangle t.1 t.2.1 t.2.2 →
      ∀ s : ℕ, (s : ℤ) = t.2.1 - l →
      ∑' e : ℕ × ℤ, (fpDist s e).toReal
...
/-- `fpDist_any_triangle_le`, original `∃`-form: delegates to the `_at` sibling
(threshold `0`). -/
theorem fpDist_any_triangle_le :
    ∃ Cthr : ℕ, ∀ n ξ : ℕ, ¬ 3 ∣ ξ →
      ...
      ∀ s : ℕ, (s : ℤ) = t.2.1 - l →
      ∑' e : ℕ × ℤ, (fpDist s e).toReal
        * Set.indicator (phaseInFamily F) 1 (n / 2 - m + e.1, l + e.2) ≤ 1 / 8 :=
  ⟨0, fpDist_any_triangle_le_at⟩
...
theorem fpDist_white_exit_deep_core (CthrO CthrT : ℕ)
    (hOut : ∀ n ξ : ℕ, ¬ 3 ∣ ξ →
      ∀ F : TriangleFamily n ξ, ∀ m : ℕ, CthrO ≤ m → m ≤ n / 2 →
      ∀ l : ℤ, 1 ≤ n / 2 - m →
      ∀ t ∈ F.T, (n / 2 - m - 1, l) ∈ triangle t.1 t.2.1 t.2.2 →
      ∀ s : ℕ, (s : ℤ) = t.2.1 - l →
      ∑' e : ℕ × ℤ, (fpDist s e).toReal
        * Set.indicator (outStripSet n) 1 (n / 2 - m + e.1, l + e.2) ≤ 1 / 8)
    (hTri : ∀ n ξ : ℕ, ¬ 3 ∣ ξ →
      ∀ F : TriangleFamily n ξ, ∀ m : ℕ, CthrT ≤ m → m ≤ n / 2 →
      ∀ l : ℤ, 1 ≤ n / 2 - m →
      ∀ t ∈ F.T, (n / 2 - m - 1, l) ∈ triangle t.1 t.2.1 t.2.2 →
      ∀ s : ℕ, (s : ℤ) = t.2.1 - l →
      ∑' e : ℕ × ℤ, (fpDist s e).toReal
        * Set.indicator (phaseInFamily F) 1 (n / 2 - m + e.1, l + e.2) ≤ 1 / 8) :
    ∀ n ξ : ℕ, ¬ 3 ∣ ξ →
      ∀ F : TriangleFamily n ξ, ∀ m : ℕ, max CthrO CthrT ≤ m → m ≤ n / 2 →
      ∀ l : ℤ, 1 ≤ n / 2 - m →
      ∀ t ∈ F.T, (n / 2 - m - 1, l) ∈ triangle t.1 t.2.1 t.2.2 →
      ∀ s : ℕ, (s : ℤ) = t.2.1 - l →
      3 / 4 ≤ ∑' e : ℕ × ℤ, (fpDist s e).toReal
...
/-- The white-exit mass of `fpDist_white_exit_deep`, symbolic (big-C campaign,
step 2): `3/4`. -/
noncomputable def p_whiteExit : ℝ := 3 / 4

/-- The threshold of `fpDist_white_exit_deep`, symbolic (big-C campaign,
step 2): `max T_outStrip 0` (the two ⅛-tail thresholds). -/
noncomputable def T_whiteExitDeep : ℕ := max T_outStrip 0

theorem p_whiteExit_ge : (51 : ℝ) / 100 ≤ p_whiteExit := by
  unfold p_whiteExit; norm_num

/-- `fpDist_white_exit_deep`, `_at` sibling: `fpDist_white_exit_deep_core` at
(`T_outStrip`, `0`), folded into `p_whiteExit`/`T_whiteExitDeep`. -/
theorem fpDist_white_exit_deep_at :
    ∀ n ξ : ℕ, ¬ 3 ∣ ξ →
      ...
      p_whiteExit ≤ ∑' e : ℕ × ℤ, (fpDist s e).toReal
        * Set.indicator (whiteStrip n ξ) 1 (n / 2 - m + e.1, l + e.2) := by
  have h := fpDist_white_exit_deep_core T_outStrip 0
    fpDist_out_of_strip_le_at fpDist_any_triangle_le_at
  unfold p_whiteExit T_whiteExitDeep
  exact h

/-- **The (7.59)-shaped deep white-exit bound**, original `∃`-form: delegates
to the `_at` sibling at `p_whiteExit = 3/4`, `T_whiteExitDeep`. -/
theorem fpDist_white_exit_deep :
    ∃ p₀ : ℝ, 51 / 100 ≤ p₀ ∧ ∃ Cthr : ℕ, ∀ n ξ : ℕ, ¬ 3 ∣ ξ →
      ...
      p₀ ≤ ∑' e : ℕ × ℤ, (fpDist s e).toReal
        * Set.indicator (whiteStrip n ξ) 1 (n / 2 - m + e.1, l + e.2) :=
  ⟨p_whiteExit, p_whiteExit_ge, T_whiteExitDeep, fpDist_white_exit_deep_at⟩
...
theorem many_triangles_white_core (p₀ : ℝ) (Cthr : ℕ) (hp₀ : 51 / 100 ≤ p₀)
  ...
  have hXe : encChainX ε p₁ ≤ Real.exp ε := encChainX_le_exp hε.le hsmall
  have hX1 : 1 ≤ encChainX ε p₁ := one_le_encChainX hε.le hp1 hsmall
  have hX0 : 0 ≤ encChainX ε p₁ := le_trans zero_le_one hX1
  ...
  have hY := encExpect_entered_le F Cthr ε p₁ hε.le hp1 hsmall hXe1 hwhite
  ...
  have hwander := encExpect_wander_le F (R - 1) Cthr ε hε.le (encChainX ε p₁)
...
theorem fpDist_height_tail_core (cL CL K : ℝ) (hcL : 0 < cL) (hCL : 0 < CL) (hK : 0 < K)
  ...
  calc ∑' e : ℕ × ℤ, (if (s : ℝ) + y ≤ (e.2 : ℝ) then fpDist s e else 0)
    ...
    _ ≤ ENNReal.ofReal (Real.exp (-(cL / 2) * y))
        ...
        exact mul_le_mul_right (mul_le_mul' hAle (le_of_eq hBle)) _
...
/-! ### PINNED Lemma 7.10 / X10a (blueprint) — restored byte-identical to `e08871e`.
The Case-3 chain consumes the `_rpow` engines below; these pins formalize the
paper's `s > m/log²m` statements (judge pass 26 repair). -/
...
paper's deep hypothesis `s > m/log²m` (`m = n/2 - j`), restored byte-identical to
...
theorem triangle_encounter_le_core (C c A₀ : ℝ)
  ...
  have hs'0 : (0 : ℝ) < (s' : ℝ) := by exact_mod_cast hs'1
  have hCle : C ≤ max C ((10 : ℝ) ^ (11 : ℕ)) := le_max_left _ _
  have h11le : (10 : ℝ) ^ (11 : ℕ) ≤ max C ((10 : ℝ) ^ (11 : ℕ)) := le_max_right _ _
  have hmaxC0 : (0 : ℝ) < max C ((10 : ℝ) ^ (11 : ℕ)) := lt_of_lt_of_le hC hCle
  by_cases hM27 : ((10 : ℝ) ^ (27 : ℕ)) ≤ (m : ℝ)
  · -- deep regime: bridge `m/log²m < s ⟹ m^{0.8} < s`, then apply the engine
    have hm0R : (0 : ℝ) < (m : ℝ) := lt_of_lt_of_le (by norm_num) hM27
    ...
    have hm08 : (m : ℝ) ^ (0.8 : ℝ) ≤ (m : ℝ) / Real.log (m : ℝ) ^ 2 := by
      ...
      calc (m : ℝ) ^ (0.8 : ℝ) * Real.log (m : ℝ) ^ 2
        ...
        _ = (m : ℝ) := by rw [← Real.rpow_add hm0R]; norm_num
    have hbridge : (m : ℝ) ^ (0.8 : ℝ) < (s : ℝ) := lt_of_le_of_lt hm08 hdeep
    have heng := hEng A hA n ξ hξ F t₀ ht₀ j l hmem s hs hbridge p s' hs'1 hs'm
    refine le_trans heng ?_
    have hd : (0 : ℝ) ≤ A ^ 2 * (1 + (p : ℝ)) / (s' : ℝ) := div_nonneg (by positivity) hs'0.le
    have he : (0 : ℝ) ≤ Real.exp (-c * A ^ 2 * (1 + (p : ℝ))) := (Real.exp_pos _).le
    have t1 : C * A ^ 2 * (1 + (p : ℝ)) / (s' : ℝ)
        = C * (A ^ 2 * (1 + (p : ℝ)) / (s' : ℝ)) := by ring
    have t2 : max C ((10 : ℝ) ^ (11 : ℕ)) * A ^ 2 * (1 + (p : ℝ)) / (s' : ℝ)
        = max C ((10 : ℝ) ^ (11 : ℕ)) * (A ^ 2 * (1 + (p : ℝ)) / (s' : ℝ)) := by ring
    rw [t1, t2]
    exact add_le_add (mul_le_mul_of_nonneg_right hCle hd)
      (mul_le_mul_of_nonneg_right hCle he)
  · -- shallow regime: `m < 10²⁷`, LHS ≤ 1 ≤ RHS
    push Not at hM27
    have hLHS1 : ∑' e : ℕ × ℤ, (fpDistPlus s p e).toReal
        * Set.indicator (bigTriangleSet F s') (1 : ℕ × ℤ → ℝ) (j + e.1, l + e.2)
        ≤ 1 := fpDistPlus_indicator_sum_le_one s p _ _
    have hAge1 : (1 : ℝ) ≤ A := le_trans hA₀ hA
    have hp1 : (1 : ℝ) ≤ 1 + (p : ℝ) := by
      have hpp : (0 : ℝ) ≤ (p : ℝ) := Nat.cast_nonneg p
      linarith
    have hAsq1 : (1 : ℝ) ≤ A ^ 2 := by nlinarith [hAge1, sq_nonneg (A - 1)]
    have hA1 : (1 : ℝ) ≤ A ^ 2 * (1 + (p : ℝ)) := by
      nlinarith [hAsq1, hp1,
        mul_nonneg (sub_nonneg.mpr hAsq1) (by linarith [hp1] : (0 : ℝ) ≤ 1 + (p : ℝ))]
    have hs'lt : (s' : ℝ) < (10 : ℝ) ^ (11 : ℕ) := by
      have h1 : (m : ℝ) ^ (0.4 : ℝ) ≤ ((10 : ℝ) ^ (27 : ℕ)) ^ (0.4 : ℝ) :=
        Real.rpow_le_rpow (by positivity) hM27.le (by norm_num)
      have h2 : ((10 : ℝ) ^ (27 : ℕ)) ^ (0.4 : ℝ) = (10 : ℝ) ^ (10.8 : ℝ) := by
        rw [← Real.rpow_natCast (10 : ℝ) 27, ← Real.rpow_mul (by norm_num : (0 : ℝ) ≤ 10)]
        norm_num
      have h3 : (10 : ℝ) ^ (10.8 : ℝ) < (10 : ℝ) ^ ((11 : ℕ) : ℝ) :=
        Real.rpow_lt_rpow_of_exponent_lt (by norm_num) (by norm_num)
      have h4 : (10 : ℝ) ^ ((11 : ℕ) : ℝ) = (10 : ℝ) ^ (11 : ℕ) := Real.rpow_natCast (10 : ℝ) 11
      calc (s' : ℝ) ≤ (m : ℝ) ^ (0.4 : ℝ) := hs'm
        _ ≤ ((10 : ℝ) ^ (27 : ℕ)) ^ (0.4 : ℝ) := h1
        _ = (10 : ℝ) ^ (10.8 : ℝ) := h2
        _ < (10 : ℝ) ^ ((11 : ℕ) : ℝ) := h3
        _ = (10 : ℝ) ^ (11 : ℕ) := h4
    have hs'maxC : (s' : ℝ) ≤ max C ((10 : ℝ) ^ (11 : ℕ)) :=
      le_of_lt (lt_of_lt_of_le hs'lt h11le)
    have hfrac : (1 : ℝ) ≤ max C ((10 : ℝ) ^ (11 : ℕ)) * A ^ 2 * (1 + (p : ℝ)) / (s' : ℝ) := by
      have hnum : (s' : ℝ) ≤ max C ((10 : ℝ) ^ (11 : ℕ)) * A ^ 2 * (1 + (p : ℝ)) := by
        nlinarith [hs'maxC, hA1, hmaxC0.le,
          mul_nonneg hmaxC0.le (sub_nonneg.mpr hA1)]
      rw [le_div_iff₀ hs'0]
      linarith [hnum]
    have hexp_nn : (0 : ℝ) ≤ max C ((10 : ℝ) ^ (11 : ℕ))
        * Real.exp (-c * A ^ 2 * (1 + (p : ℝ))) :=
      mul_nonneg hmaxC0.le (Real.exp_pos _).le
    calc ∑' e : ℕ × ℤ, (fpDistPlus s p e).toReal
            * Set.indicator (bigTriangleSet F s') (1 : ℕ × ℤ → ℝ) (j + e.1, l + e.2)
          ≤ 1 := hLHS1
      _ ≤ max C ((10 : ℝ) ^ (11 : ℕ)) * A ^ 2 * (1 + (p : ℝ)) / (s' : ℝ) := hfrac
