...
`logProb A (Finset.Icc 1 x)` as `x → ∞`, where `Finset.Icc 1 x` is the window
`ℕ+ ∩ [1, x]`. Predicate form so it makes sense even when the limit does not
exist; assert a specific value `d` via `HasLogDensity A d`. -/
